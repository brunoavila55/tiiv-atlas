...
export function TenantsPage(){
  ...
  const saveBranding=async()=>{if(!branding)return;const data=new FormData();data.append('brand_name',brandName);if(logo)data.append('logo',logo);if(favicon)data.append('favicon',favicon);try{const response=await fetch(`/api/v1/tenants/${branding.id}/branding`,{method:'PUT',credentials:'include',body:data});const body=await response.json().catch(()=>null);if(!response.ok){const fallback=response.status===413?'Branding files are too large. Use a logo up to 2 MB and a favicon up to 256 KB.':'Could not update branding';throw new Error(body?.error?.message||fallback)}setBranding(null);await load()}catch(reason){setError(reason instanceof Error?reason.message:'Could not update branding')}};
